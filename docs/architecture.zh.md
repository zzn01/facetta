...
| `dict.go` | 53 | 字符串 ↔ uint32 id 字典(整数维附带解析值数组),发布后不可变 |
...
(`Dim.Type`)的维度上,身份就是 int64 值:每个写入边界(构建、归并、Apply)都在
字典插入前把拼写规范化为纯十进制形式(并拒绝非整数输入),查询边界则把条件值
规范化进一个栈上缓冲区,零分配地完成查找。字典附带一个平行的 `vals []int64`,
因此查询时一次范围检查就是一次 id 查找加两次整数比较 —— 不扫字典、不用位图、
零分配,维度身份中也不存在任何浮点精度悬崖。快照格式不受影响(vals 在加载时
重新推导),但加载时会校验整数维的既有词条均为规范形式,拒绝在该维声明为
DimInt 之前写出的快照。
