...
| `BenchmarkApply1K` (1000-record batch onto 1M rows) | ~1.4 ms/op averaged over the delta-growth cycle up to the 50k compaction threshold (Apply copies the delta columns; the tuple index is map-cloned, not rebuilt) — see note below | ~3.9 MB/op | ~1.1k allocs/op |
...
`BenchmarkApply1K`'s history needs a correction: earlier tables reported ~4.5 ms and one session even ~7-11 ms with heavy run-to-run swings, and the swing was at first misread as a write-path regression. Controlled A/B runs across master and both feature branches showed identical per-op work (byte-identical B/op and allocs/op) — the instability came from the benchmark itself: its setup regenerated the full record prefix per batch, O(b.N²) work that both risked multi-minute runaways at high iteration counts and polluted the timed region's GC state enough to double the reported ns/op. With the setup fixed to generate the batches in one linear pass, the number is stable (~1.4 ms/op, ±3% across runs). Note the reported ns/op still depends on where `b.N` lands in the delta-growth cycle, so cross-run comparisons should pin `-benchtime=Nx`. `BenchmarkReplaceAll1M`, by contrast, is a measurement artifact worth flagging rather than a regression: a default single-count run calibrates it to one iteration (`b.N=1`, no in-run averaging), which measured ~1.85 s/op — but `-count=3` recalibrates it to 2-3 iterations and settles at ~481 ms/op, in line with the ~570 ms baseline; the table above uses the corrected, multi-iteration number.
